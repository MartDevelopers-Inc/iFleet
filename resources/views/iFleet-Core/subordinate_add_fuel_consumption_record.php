<?php
    ...
    if(isset($_POST['add_fuel_record']))
    {
        ...
        else
        {
            ...
            $sql="SELECT * FROM  iFleet_fuel_consumption WHERE  f_code ='$f_code'";
            $res=mysqli_query($mysqli,$sql);
            if (mysqli_num_rows($res) > 0)
            {
            $row = mysqli_fetch_assoc($res);
            $err =  "Fuel Consumption Record With That Number Exists";
            }
        }
            ...
            $postQuery="INSERT INTO iFleet_fuel_consumption (f_code, f_veh_reg_no, f_veh_name, f_type, f_consumed) VALUES (?,?,?,?,?) ";
            $postStmt = $mysqli->prepare($postQuery);
            //bind paramaters
            $rc=$postStmt->bind_param('sssss', $f_code, $f_veh_reg_no, $f_veh_name, $f_type, $f_consumed);
            $postStmt->execute();
            //declare a varible which will be passed to alert function
            if($postStmt)
            {
                $success = "Fuel Added" && header("refresh:1; url=subordinate_fuel_management.php");
            }
            else 
            {
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item"><a href="subordinate_fuel_management.php">Fuel Consumption</a></li>
                    ...
                    <div class="card-body">
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" name="f_code"  value="iFleet-<?php echo $alpha;?>-<?php echo $beta;?>" class="form-control">
                            </div>
                            <div class="form-group col-md-6">
                                <label for="exampleInputPassword1">Fuel Type</label>
                                <input type="text" class="form-control"  name="f_type">
                            </div>
                        </div>
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <input type="text" name="f_consumed"  class="form-control">
                            </div>
                        </div>
                    </div>
                    <!-- /.card-body -->
                    <div class="card-footer">
                        <button type="submit" name="add_fuel_record" class="btn btn-outline-primary"><i class="fas fa-save"></i> Save</button>
                    </div>
                </form>
            </div>
            <!-- /.card-body -->
